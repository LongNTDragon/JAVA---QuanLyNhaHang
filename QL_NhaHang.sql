...
('NCC03', N'Chợ Bà Chiểu', N'Một nơi nào đó ở TPHCM', '0987654321'),
('NCC04', N'Chợ Bến Thành', N'Một nơi nào đó ở TPHCM', '0246897531')
...
INSERT INTO MATHANG
VALUES
('MH01', N'Đồ nguội', 50000, 1, 'DM01', 'DVT08', 'NCC04'),
('MH02', N'Rau câu', 50000, 1, 'DM02', 'DVT08', 'NCC04'),
('MH03', N'Pubding', 50000, 1, 'DM02', 'DVT08', 'NCC04'),
('MH04', N'Khoai tây chiên', 40000, 1, 'DM03', 'DVT08', 'NCC03'),
('MH05', N'Vũ nữ chân dài', 120000, 1, 'DM03', 'DVT08', 'NCC03'),
('MH06', N'Bòa xào hành', 120000, 1, 'DM04', 'DVT08', 'NCC03'),
('MH07', N'Mực xào xa tế', 120000, 1, 'DM04', 'DVT08', 'NCC03'),
('MH08', N'Lẩu Thái', 120000, 1, 'DM05', 'DVT08', 'NCC03'),
('MH09', N'Lẩu nắm', 120000, 1, 'DM05', 'DVT08', 'NCC03'),
('MH10', N'Bò nướng lá ngón', 100000, 1, 'DM06', 'DVT08', 'NCC04'),
('MH11', N'Súp cua King', 100000, 1, 'DM07', 'DVT08', 'NCC04'),
('MH12', N'Van Đà Lạt', 500000, 120, 'DM08', 'DVT01', 'NCC02'),
('MH13', N'Saigon', 18000, 120, 'DM09', 'DVT07', 'NCC01'),
('MH14', N'Tiger', 25000, 120, 'DM09', 'DVT07', 'NCC01'),
('MH15', N'Nước suối', 10000, 120, 'DM10', 'DVT01', 'NCC01')
GO
...
--*                            TABLE BAN                              *-- NO COMMIT
--*********************************************************************--
CREATE PROC sp_InsertBAN @maBAN VARCHAR(10), @tenBAN NVARCHAR(50), @soGhe INT
AS
	BEGIN
...
CREATE PROC sp_InsertMH @maMH VARCHAR(10), @tenMH NVARCHAR(50), @giaMH FLOAT, @soL INT, @maDM VARCHAR(10), @maDVT VARCHAR(10), @maNCC VARCHAR(10)
AS
	BEGIN
		INSERT INTO MATHANG
		VALUES
		(@maMH, @tenMH, @giaMH, @soL, @maDM, @maDVT, @maNCC)
	END
GO

CREATE PROC sp_UpdateMH @maMH VARCHAR(10), @tenMH NVARCHAR(50), @giaMH FLOAT, @soL INT, @maDM VARCHAR(10), @maDVT VARCHAR(10), @maNCC VARCHAR(10)
AS
	BEGIN
		UPDATE MATHANG
		SET TENMH = @tenMH, GIAMH = @giaMH, SOLUONG = @soL, MADM = @maDM, MADVT = @maDVT, MANCC = @maNCC
		WHERE MAMH = @maMH
	END
GO

CREATE PROC sp_DeleteMH @maMH VARCHAR(10)
AS 
	BEGIN
		DELETE MATHANG
		WHERE MAMH = @maMH
...
CREATE PROC sp_FindMH @INFO NVARCHAR(100)
AS
	BEGIN
		SELECT MH.MAMH, TENMH, GIAMH, SOLUONG, TENDM, TENDVT, TENNCC
		FROM MATHANG MH, DVT, DANHMUCHANG, NHACUNGCAP
		WHERE MH.MADVT = DVT.MADVT AND MH.MADM = DANHMUCHANG.MADM AND MH.MANCC = NHACUNGCAP.MANCC AND TENMH LIKE '%' + @INFO + '%'
	END
GO

CREATE PROC sp_FindMHByKey @key VARCHAR(10)
AS
	BEGIN
		SELECT MH.MAMH, TENMH, GIAMH, SOLUONG, TENDM, TENDVT, TENNCC
		FROM MATHANG MH, DVT, DANHMUCHANG, NHACUNGCAP
		WHERE MH.MADVT = DVT.MADVT AND MH.MADM = DANHMUCHANG.MADM AND MH.MANCC = NHACUNGCAP.MANCC AND MH.MAMH = @key
	END
GO

CREATE PROC sp_ShowAll
AS
	BEGIN
		SELECT MH.MAMH, TENMH, GIAMH, SOLUONG, TENDM, TENDVT, TENNCC
		FROM MATHANG MH, DVT, DANHMUCHANG, NHACUNGCAP
		WHERE MH.MADVT = DVT.MADVT AND MH.MADM = DANHMUCHANG.MADM AND MH.MANCC = NHACUNGCAP.MANCC
	END
GO

CREATE PROC sp_SelectLastMaMH
AS
	BEGIN
		SELECT TOP 1(SUBSTRING(MAMH,2,5)) FROM MATHANG ORDER BY MAMH DESC
	END
GO